		...
		menufunctions: function(){
			...
			if( $body.hasClass('device-xl') || $body.hasClass('device-lg') ) {
				$('#primary-menu.sub-title > ul > li').hover(function() {
					...
					$(this).prev().css({ backgroundImage : 'url("imgs/icons/menu-divider.png")' });
				});
...
// ---------------------------------------------------------------------------------------------------------------------//
			$(".plus-nbjh").off( 'click' ).on( 'click', function(){
				var productQuantity = $(this).parents('.quantity').find('.nbjh').val(),
					quantityStep = $(this).parents('.quantity').find('.nbjh').attr('step'),
					intRegex = /^\d+$/;
				...
				if( intRegex.test( productQuantity ) ) {
					var productQuantityPlus = Number(productQuantity) + Number(quantityStep);
					$(this).parents('.quantity').find('.nbjh').val( productQuantityPlus );
				} else {
					$(this).parents('.quantity').find('.nbjh').val( Number(quantityStep) );
				}

				return false;
			});

			$(".minus-nbjh").off( 'click' ).on( 'click', function(){
				var productQuantity = $(this).parents('.quantity').find('.nbjh').val(),
					quantityStep = $(this).parents('.quantity').find('.nbjh').attr('step'),
					intRegex = /^\d+$/;
				...
				if( intRegex.test( productQuantity ) ) {
					if( Number(productQuantity) > 1 ) {
						var productQuantityMinus = Number(productQuantity) - Number(quantityStep);
						$(this).parents('.quantity').find('.nbjh').val( productQuantityMinus );
					}
				} else {
					$(this).parents('.quantity').find('.nbjh').val( Number(quantityStep) );
				}
